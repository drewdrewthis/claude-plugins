---
name: what-do-i-know
description: "The gateway to everything the codex knows — procedures, decisions, solutions, references, principles, research, not procedures alone. Use when you (or the user) ask 'is there a procedure for X', 'how do we do X', 'what is X', 'why did we decide X', 'what happened when X', AND — equally — whenever you are ABOUT TO PERFORM a documented operation ('tag the issue', 'label this PR', 'cut a release', 'prune/launch/stop a session', 'drive boxd/fleet'). An imperative 'do X' is still a trigger: learn what governs it BEFORE executing. Invoke with TWO parts separated by '|': (1) GOAL — one concrete sentence stating what you are about to DO, or what you want to KNOW, and where (repo/surface); (2) TERMS — the full search vocabulary: every concept the answer might touch, each with 2–3 synonyms, corpus words included (proc, fm, evolve, squash), phrases hyphenated. The list must be COMPLETE because there is exactly one query. Never answer or execute github/slack/release/fleet/boxd operations from memory."
user-invocable: true
...
The caller is about to act, or wants to know something, and has handed you a
GOAL and a TERM LIST. You turn them into one question, run the two-stage
retrieval pipeline ONCE, and hand back the grounded proposal it returns.
...
- Never act on the goal, invent an answer not grounded in a retrieved record,
  omit a trap because it looks unlikely, or flatten a contradiction between
  two records — report both and say they disagree.
- **Absence is a finding, with an instruction.** "Nothing governs this" is a
  real answer: tell the caller to improvise and to draft the record once the
  improvisation has actually worked (or the fact is confirmed), via
  `/update-records` — procedure, decision, or solution, whichever kind fits.
  A record written before its claim is confirmed is a guess with an id on it.
