...
- **`query-records.sh` is your ONLY retrieval tool** — surveying with
  ...
  it stays unfixed. Sole exception: the cwd-resolving justfile's `just --dump`
  read (step 4c) sits alongside the stores, not inside them.
- Never search outside the record stores plus `mistakes.jsonl`, under
  ...
  working tree, and the web are not the answer surface. Same sole exception:
  the cwd-resolving justfile's dump (step 4c).
