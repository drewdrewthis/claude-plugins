...
n=${#segments[@]}
if [ "$n" -eq 1 ]; then
  printf '%s' "${segments[0]}" | grep -Eq '^(just|cd|pwd|echo|ls|cat|command[[:space:]]+-v|which)([[:space:]]|$)' || all_ok=0
elif [ "$n" -gt 1 ]; then
